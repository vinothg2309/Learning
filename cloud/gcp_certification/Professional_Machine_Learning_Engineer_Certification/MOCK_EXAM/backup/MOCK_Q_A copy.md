...
- [ ] A. Use LINEAR_REG first for baseline (simpler, interpretable); if performance is poor, try BOOSTED_TREE for non-linear patterns
...
- [ ] A. Use ML.STANDARD_SCALER to normalize features to mean=0, std=1; ensures features contribute equally to loss
...
- [ ] A. Class imbalance: if 98% don't churn, predicting "no churn" always gives 98% accuracy but 0% recall. Set class_weight=BALANCED in training to penalize false negatives more
...
- [ ] A. No; high precision alone doesn't guarantee good model. If recall is low (0.6), you're missing 40% of true positives. F1 (harmonic mean) better balances precision-recall trade-off
...
- [ ] A. Use ML.FEATURE_CROSS to create interaction terms (e.g., 'senior_C'). Add crossed features to model; captures non-linear relationships
...
- [ ] A. Yes; if data shows trend (sales growing) or seasonality (peaks in Dec), include them. ARIMA with trend + seasonality captures these patterns. Check ACF/PACF plots
...
- [ ] A. debt_ratio was most important in the prediction decision (80% contribution). For this customer, high debt_ratio likely triggered rejection. Income had minimal impact
...
- [ ] A. Try lower learning_rate (0.01) first; high learning rate may cause oscillation. If validation loss is still high, increase. Use grid search: test multiple rates and pick best
...
- [ ] A. Depends on context. If typical house price is 300K, MAE=15K is 5% error (good). If typical price is 100K, 15% error (poor). Compare to baseline (always predicting mean price)
...
- [ ] A. Overfitting: model memorized training data. Diagnose via: (1) compare train vs test loss (gap=overfitting), (2) reduce model complexity, (3) regularization (add L1/L2 penalty), (4) more data. In BQML, set input_label_cols, use balanced data
...
- [ ] A. Temporal split: train on weeks 1–90, test on weeks 91–104. Never use future data for training (data leakage). Evaluate: does model predict weeks 91–104 well?
...
- [ ] A. Yes; manually creating polynomial/interaction features extends LINEAR_REG expressiveness. y = w1*age + w2*age^2 + w3*age*income + ... captures non-linearity. But BOOSTED_TREE does this automatically
...
- [ ] A. Use ML.PREDICT for batch: predict on 1M rows in BigQuery in minutes. Use online endpoint if you need <100ms per-request latency. Batch is cheaper; online is faster
...
- [ ] A. Name models with version: model_v1, model_v2. Store both in BigQuery ML. Keep model_v1 in production, serve model_v2 in canary (5% traffic). After validation, promote v2. Keep old versions for rollback
...
- [ ] A. L2 regularization (Ridge): adds penalty for large weights. In BQML, set L2_REG parameter. L1 (Lasso) zeroes out weak features. Adjust regularization strength until train-test gap shrinks
- [ ] B. Remove features
...
- [ ] A. Yes; lower threshold (e.g., 0.01) flags more transactions as fraud (catches more true fraud but more false alarms). Use ROC curve (ML.ROC_CURVE) to find threshold balancing precision/recall for your use case
...
- [ ] A. Class imbalance: if C is rare in data (5%), model optimizes for majority classes. Check per-class accuracy via confusion matrix. Use BALANCED class weights or collect more C samples
...
- [ ] A. Export BQML model to Cloud Storage (as .json or model artifact). Import to Vertex AI Model Registry. Deploy endpoint. BQML models are standalone; no framework-specific code needed
...
- [ ] A. Yes; BQML ARIMAPLUS allows external regressors. Include marketing spend (strong signal) and competitor price. Model: sales ~ ARIMA + external_regressors. Improves accuracy if external data is predictive
...
- [ ] A. Use K-MEANS clustering (unsupervised). No labels needed; groups similar customers. LINEAR_REG is supervised (needs target). Create features (spending, frequency, RFM). Run K-MEANS with k=5. Analyze clusters
...
- [ ] A. Define 5 components (functions wrapped with @dsl.component). Connect via inputs/outputs. Use dsl.Dag to define execution order. Add conditional gate after evaluate: if metrics_pass, deploy; else skip. Compile to YAML
...
- [ ] A. Define output parameter in A: model_artifact = Output[Model]. Component B: input model_artifact = Input[Model]. KFP automatically handles artifact storage (Cloud Storage) and passing. Use @dsl.component decorator
...
- [ ] A. Use dsl.Condition: @dsl.component def deploy(acc) and @dsl.component def retrain. In DAG: with dsl.Condition(evaluate.outputs['accuracy'] > 0.95): deploy_task. Else: retrain_task
...
- [ ] A. Enable caching: set enable_caching=True on steps 1–3 that don't change. KFP skips re-execution if inputs are identical, reuses cached outputs. Saves 30–60 min. Only step 4 (training) re-runs
...
- [ ] A. Parallelize training: use @dsl.component def train_arch1, train_arch2, ... inside a loop or fan-out. KFP runs all 5 in parallel (on separate workers). Total: max(30) + 5 = 35 min. Requires sufficient compute quota
- [ ] B. Serial only
...
- [ ] A. Use Output[Model], Output[Metrics], Output[ClassificationMetrics]. KFP auto-tracks artifact type, stores in Artifact Registry, records metadata (model version, accuracy). Enables lineage (which data trained which model)
...
- [ ] A. Use pipeline parameters: @dsl.pipeline(name='train', default_params={'learning_rate': 0.01, ...}). Pass at execution: Compiler().compile(pipeline, 'pipeline.yaml'); Run with params override. Enables experimentation
...
- [ ] A. Yes; use @dsl.component(..., retry=3) to retry failed component. Exponential backoff (wait 1s, 2s, 4s). If 3 retries fail, pipeline fails. Handles transient failures gracefully
- [ ] B. No retries
- [ ] C. Fail immediately
- [ ] D. Infinite retries
...
- [ ] A. Define pipeline in KFP. Deploy to Vertex AI Pipelines. Create schedule trigger (Cloud Scheduler): run pipeline every 30 days. Monitor drift; if detected, trigger retraining immediately. Logs training history in Vertex ML Metadata
- [ ] B. Manual retraining
- [ ] C. No scheduling
...
- [ ] A. Add TFDV (TensorFlow Data Validation) component: compute statistics, generate schema, validate data quality. Reject data if nulls exceed threshold. Fail pipeline, alert team. Prevents bad data→bad model
- [ ] B. No validation
- [ ] C. Manual inspection
- [ ] D. Ignore nulls
...
- [ ] A. Register all models in Vertex Model Registry: set v2 as "Candidate" (shadow), v1 as "Production" (live). After validation, promote v2 to Production, archive v1. Each version has metadata (training data, metrics, lineage)
...
- [ ] A. Enable verbose logging in KFP. Check Vertex Pipelines dashboard → failed run → step 3 → logs. Observe OOM; increase memory or reduce batch size. Re-run. Logs are persistent in Cloud Logging
...
- [ ] A. Use orchestration: Pipeline B depends on Pipeline A (output artifact of A is input to B). Or use Cloud Workflows to chain pipelines: Run A, wait for completion, then run B. Prevents race conditions
...
- [ ] A. Add component: fetch features from Vertex Feature Store using feature view (pre-defined grouping). Component outputs dataframe. Training component uses it. Enables point-in-time lookups (features as they were at training time)
...
- [ ] A. Log metrics to Vertex ML Metadata using @dsl.component output (Metrics artifact). Vertex Pipelines tracks all runs, visualizes trends. Alert if metrics drop >5%. Enables continuous monitoring and regression detection
- [ ] B. CSV file manually
- [ ] C. No tracking
- [ ] D. Ignore metrics
...
- [ ] A. Build custom container image (Dockerfile, push to Artifact Registry). In KFP component, specify @dsl.component(base_image='gcr.io/my-project/my-image:latest'). KFP uses your image for that component
...
- [ ] A. Schedule: Cloud Scheduler (cron). Event: Cloud Storage trigger (notify Cloud Pub/Sub when file uploaded) → Cloud Functions → trigger pipeline. Hybrid: schedule + event-driven automation. Provides flexibility
- [ ] B. Schedule only
- [ ] C. Event only
- [ ] D. Manual triggering
...
- [ ] A. Pipeline stores model versions in Model Registry. Rollback: endpoint → traffic to v1. Simultaneously, run pipeline to fix data issue, retrain, redeploy v2. Use versioning + traffic splitting for quick recovery
...
- [ ] A. Optimize: (1) use preemptible VMs (70% cheaper, okay for training), (2) use smaller GPU (T4→K80 if throughput allows), (3) profile code (data loading is bottleneck?), (4) cache intermediate outputs (skip redundant steps), (5) cloud build on-demand (no reserved capacity). Target: $2K/month
- [ ] B. No cost control
...
- [ ] A. Vertex Pipelines auto-tracks: execution metadata (user, timestamp), input datasets (lineage), output models (version). ML Metadata stores all. Export audit trail: query Metadata API, generates report. Models tagged with compliance metadata
- [ ] B. No audit trail
...
- [ ] A. Use tumbling (1-min, fixed) for regular metrics. Sliding if you want smooth overlapping views (trade-off: 2x computation). Session if idle periods matter (e.g., user sessions in logs). For sensors: tumbling is efficient
...
- [ ] A. Yes; use allowed_lateness=30s in Dataflow. When late data arrives within 30s, re-emit window. If you care about completeness, keep window open. If punctuality matters, close window early. Trade-off: lateness vs completeness
- [ ] B. Discard late data
...
- [ ] A. Depends on use case. Default: emit on watermark (complete data, final). Add trigger: AfterWatermark().with_early_firings() to emit early (e.g., every 5s). Trade-off: low latency (early) vs high accuracy (final). Choose by SLA
- [ ] B. Never emit early
- [ ] C. Always emit early
- [ ] D. No triggers
...
- [ ] A. Load tier reference data into side input (broadcast to all workers). In pipeline: main stream joined with side input via ParDo. Efficient (small lookup table broadcast once). Reload when reference updates. Avoids expensive stream-stream join
...
- [ ] A. Use Cloud Dataflow template: Pub/Sub to BigQuery. Dataflow auto-scales workers based on Pub/Sub backlog (Stackdriver metrics). Pipeline: Read(Pub/Sub) → Filter → Window → Aggregate → Write(BigQuery). Highly available, fault-tolerant
- [ ] B. Single worker
- [ ] C. Manual scaling
...
- [ ] A. Use ParDoFn with @process_bundle and self.state (per-worker state) or StateValueSpec for keyed state. For user-level aggregation: use stateful ParDo. State is fault-tolerant (backed by Dataflow state service). Caution: state per key must fit in memory
...
- [ ] A. Use BeamPipeline with multiple Read sources (Cloud Storage, Pub/Sub, BigQuery) and Write sinks. Branch pipeline: Read → Filter/Transform → Write to different sinks. Dataflow handles orchestration. Each sink runs independently (fan-out)
...
- [ ] A. Backpressure (flow control): Dataflow slows down source reading to match processing speed. Backlog stabilizes (doesn't grow indefinitely). Auto-scaling helps: add more workers to process faster. Monitor: if CPU <50%, add workers; if CPU >90%, investigate bottleneck
...
- [ ] A. Create Dataflow template: parameterize (template parameters for topic, table, filter). Package template in Cloud Storage. Execute via API with different params. Dataflow launches workers, runs pipeline with params. Enables self-service for non-engineers
...
- [ ] A. Yes; use exactly-once semantics. Dataflow + BigQuery streaming inserts support exactly-once (deduplication by insert ID). If duplicate detected, BigQuery discards. Alternatively, use ParDo with state to track seen IDs (memory-bounded dedup). Prevents double-counting
- [ ] B. No deduplication
...
- [ ] A. Yes; Beam unified: batch (bounded sources like BigQuery) and streaming (unbounded like Pub/Sub) use same APIs. Pipeline logic is identical. Dataflow runner selects batch vs streaming execution. One codebase, dual deployment. Reduces maintenance
...
- [ ] A. Increase parallelism: add more workers (auto-scaling), increase bundles per worker. Optimize DoFn: batch operations (vectorized), cache expensive calls. Use profiler to identify bottleneck (I/O vs CPU). Typical speedup: 5–10x with optimization
- [ ] B. Single worker
- [ ] C. No optimization
...
- [ ] A. Use dead letter pattern: ParDo with TryCatch. Try: process event. Catch: write to dead letter queue (Cloud Pub/Sub topic or BigQuery table). Main pipeline continues (doesn't fail on errors). Monitor dead letter queue; fix bad records offline
- [ ] B. Crash on error
- [ ] C. Ignore errors
- [ ] D. No error handling
...
- [ ] A. Filter(lambda x: "bot" not in x["user_agent"] and x["ip"] not in INTERNAL_IPS) → ParDo(count per URL) → Write. Filters reduce data early (save bandwidth). ParDo applies transformations. Early filtering is critical for large streams
- [ ] B. No filtering
...
- [ ] A. Stream-table join: Load user profile into side input (reference table, updated hourly). Join stream A with side input via ParDo. Efficient. Alternatively, stream-stream join (windowed) if both unbounded. Trade-off: side input is simple but may be stale; stream-stream is live but expensive
- [ ] B. No enrichment
...
- [ ] A. CPU-based autoscaling: target 70% CPU. At 100K events: 10 workers, ~10% CPU (too much spare capacity). At 1M events: scale to 100 workers, hit 70% target. Set min=5, max=200. Monitor: watch Stackdriver for CPU, worker count trends
- [ ] B. No autoscaling
- [ ] C. Fixed workers
- [ ] D. Manual adjustment
...
- [ ] A. Check Dataflow dashboard: element latency (read, process, write), lag (Pub/Sub backlog). If read latency high: source issue. If process high: DoFn bottleneck (profile). If write high: sink issue (BigQuery throttle). Set alert: if element latency >10min, notify. Investigate
- [ ] B. Ignore latency
- [ ] C. No monitoring
- [ ] D. Manual checks
...
- [ ] A. Batch requests: instead of 1 inference per record, batch 100 records per request. Call inference service with batch. Throughput improves 10x (1 call vs 100). Use ParDo with bundle-level batching. Caution: increased latency due to batching (acceptable?)
- [ ] B. Sequential calls
- [ ] C. No batching
- [ ] D. Remove inference
...
- [ ] A. Dataflow checkpoints state periodically. On crash, resume from last checkpoint (event 2). Events [3, 4, 5] reprocessed. Duplicate event 2's output (at-least-once semantics). For exactly-once, use idempotent writes (BigQuery deduplication) or transactions
...
- [ ] A. Use Custom Training: define training script (train.py), specify GPU (A100 with 40GB). Vertex AI runs script on GKE worker. More control than AutoML. For simple models, AutoML is faster; for custom architectures, use Custom Training
- [ ] B. AutoML always
- [ ] C. Only CPU training
- [ ] D. No GPU support
...
- [ ] A. Use Vertex AI Hyperparameter Tuning (HPT): specify parameter ranges, metric (validation_loss). Tuning runs parallel trials (e.g., 10 workers test different params). Best params selected. Saves time (parallel) vs manual grid search. Trade-off: cost (many trials)
...
- [ ] A. Evaluate on held-out test set (unseen data). Check fairness (accuracy per demographic). Test edge cases (empty input, extreme values). Monitor inference latency and throughput. Test rollback procedures. Only deploy if test accuracy >90% and fairness gaps <5%
...
- [ ] A. Create endpoint with traffic split: 5% → v2, 95% → v1. Monitor v2 metrics (latency, errors). If good, increase to 25% v2. A/B test: measure which version has better CTR. Gradually shift (canary deployment). Rollback easy if v2 fails
...
- [ ] A. Use 2: Online endpoint for web app (Vertex AI Prediction, low-latency). Batch endpoint (Vertex AI Batch Prediction, cost-effective, runs job overnight). Same model, different serving modes. Reduces cost (pay-per-prediction online, flat batch job cost)
- [ ] B. Batch only
- [ ] C. Online only
...
- [ ] A. Data drift: input distribution shifted (new data differs from training data). Age distribution changed, or credit scores shifted. Detect via monitoring: compare new data distribution to training baseline (Kolmogorov-Smirnov test, chi-square). Alert if drift >threshold. Trigger retraining with new data
- [ ] B. No problem
- [ ] C. Model is broken
- [ ] D. Ignore drift
...
- [ ] A. Enable prediction logging: Vertex AI auto-logs predictions to BigQuery (features, predicted label, confidence). Link to ground truth labels (join on customer ID, date). Compute accuracy on logged predictions. Use for retraining with fresh, labeled data
- [ ] B. No logging
- [ ] C. Manual tracking
...
- [ ] A. Create prediction container: (1) write predictor.py (load model, define predict(instance)), (2) Dockerfile (FROM python:3.9, COPY model.pkl, COPY predictor.py), (3) build and push to Artifact Registry, (4) create endpoint with custom container. Vertex AI runs container, handles autoscaling
...
- [ ] A. Feature latency is bottleneck (400ms). Cache features: pre-load customer features to in-memory cache (Redis) before prediction. Or batch: collect requests, predict 100 at once (amortize latency). Target: <100ms latency. A/B test: does caching improve UX?
- [ ] B. No optimization
...
- [ ] A. Endpoint has v2 deployed. Rollback: re-create endpoint with v1 model (or traffic split 100% → v1). Takes minutes. Keep v2 registered in Model Registry (version history). Diagnose v2 issue, fix, retrain, redeploy. Versioning enables fast recovery
- [ ] B. Can't rollback
...
- [ ] A. Enable autoscaling: set min=2, max=20 replicas. Target CPU 70% or request latency <1s. Vertex AI auto-scales: at 1K req/sec, scales to 10 replicas (CPU ~70%), latency <1s. Cost: pay per replica per hour (only when needed). Monitor scaling lag (usually <1 min)
- [ ] B. Fixed 2 replicas
- [ ] C. No autoscaling
...
- [ ] A. Define request schema in custom predictor.py: parse CSV, convert to JSON, pass to model. Response: JSON output. Or pre-process: client converts CSV to JSON before API call. Custom container handles format conversion (flexibility)
...
- [ ] A. Yes; Batch Prediction: load 100M rows from BigQuery, run prediction job (parallel, serverless), write results back to BigQuery. Cost: <$100, time: ~2 hours. Way cheaper than online (would cost $1K+). Use for offline scoring (customer lists, reports)
- [ ] B. Online endpoint
- [ ] C. Manual scoring
- [ ] D. Can't score 100M
...
- [ ] A. Use Vertex AI Model Registry: upload each model with metadata (version, metrics, training dataset, responsible team). Each model version has: artifact (Cloud Storage), lineage (which data trained it), evaluation results. Query registry: find best model by metric, rollback to older version, compare versions
- [ ] B. No organization
...
- [ ] A. Set up pipeline: (1) monthly trigger (Cloud Scheduler), (2) pipeline fetches latest data, trains model, evaluates. If accuracy >95% and improves by >2%, promote to production. If accuracy drops >2%, alert team (manual review). Use ML Metadata to track all experiments
- [ ] B. Manual retraining
- [ ] C. Never retrain
...
- [ ] A. Yes; shadow deployment: route traffic to v1 (primary) and v2 (shadow). v2 predictions logged but NOT returned to users. Compare v2 predictions to v1. If v2 metrics (latency, errors) good, promote to production. Enables risk-free testing of new models
- [ ] B. No shadow mode
...
- [ ] A. Techniques: (1) quantization (reduce float32 to int8, 4x smaller), (2) pruning (remove weak weights), (3) distillation (train smaller student model on large teacher model). Result: 50MB model, 200ms inference. Trade: accuracy drop ~1–2%. Test on device before deploy
- [ ] B. No compression
- [ ] C. Large models only
- [ ] D. Accept 500MB
...
- [ ] A. Model Card: document model details (training data, features, performance metrics, fairness evaluation, limitations, recommended use cases). Store in Model Registry. Share with stakeholders. Enables transparency, reproducibility, and responsible deployment. Standard practice (Google, Hugging Face use Model Cards)
- [ ] B. Not needed
...
- [ ] A. Partition by date (ingestion time or specific column). BigQuery only scans relevant date partitions (reduces data scanned, faster queries, lower cost). Instead of scanning 5TB, scan ~100GB per month. Use _PARTITIONTIME pseudo-column for queries. Partition pruning is automatic
- [ ] B. No partitioning
...
- [ ] A. Clustering collocates similar rows (country/user_id) on same storage blocks. Query filters: scans only relevant blocks (95% fewer blocks touched). Faster + cheaper. Combine: partition by date, cluster by [country, user_id]. Clustered tables auto-maintain (no manual rebuild)
- [ ] B. No benefit
...
- [ ] A. Window function: SELECT customer_id, date, amount, SUM(amount) OVER (PARTITION BY customer_id ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as running_total. Efficient (single scan), avoids self-join
...
- [ ] A. BigQuery auto-chooses: if B is small (<10GB), broadcast join (copy B to all workers, faster). If both large, sort-merge (sort both, merge). Explicit hint: use INNER JOIN (A, B) or /* @broadcast(B) */ to force broadcast. For your case (1B vs 1M), broadcast is efficient
- [ ] B. Always sort-merge
- [ ] C. No optimization
- [ ] D. Same speed
...
- [ ] A. Yes; WITH cte AS (SELECT customer_id, SUM(amount) as total FROM sales GROUP BY customer_id), ranked AS (SELECT *, ROW_NUMBER() OVER (ORDER BY total DESC) as rank FROM cte) SELECT * FROM ranked WHERE rank <= 100. CTEs are readable and often faster (materialized or optimized)
...
- [ ] A. Use UNION ALL (faster): concatenate both tables without deduplication. Then aggregate: SELECT region, SUM(revenue) FROM (SELECT * FROM us_sales UNION ALL SELECT * FROM intl_sales) GROUP BY region. UNION ALL is usually correct (no duplicates expected). UNION removes duplicates (slower)
- [ ] B. UNION only
- [ ] C. Separate queries
- [ ] D. UNION impossible
...
- [ ] A. Yes; GROUP BY treats NULL as a group. Result has row with region=NULL, count=0.1M (10% of rows). If you want to exclude NULLs: WHERE region IS NOT NULL GROUP BY region. Or: COALESCE(region, 'Unknown') GROUP BY ... to aggregate NULLs into 'Unknown'
...
- [ ] A. FLATTEN and aggregate: WITH flattened AS (SELECT customer_id, item.price * item.qty as item_value FROM sales, UNNEST(orders) as order, UNNEST(order.items) as item) SELECT customer_id, SUM(item_value) as total_revenue FROM flattened GROUP BY customer_id. UNNEST denormalizes nested arrays
- [ ] B. No nested queries
...
- [ ] A. Use APPROX_COUNT_DISTINCT(): fast approximate result (usually within 1% error) in <1 second. Or HyperLogLog sketch (APPROX_QUANTILES for percentiles). Trade: accuracy for speed. For most metrics, approximate is sufficient
- [ ] B. EXACT COUNT only
- [ ] C. No approximation
- [ ] D. Accept 10 min
...
- [ ] A. Create materialized view: CREATE MATERIALIZED VIEW sales_by_region_date AS (SELECT...). BigQuery stores pre-computed result. Query MV instead of raw table (instant, costs 0 bytes scanned). MV auto-refreshes (can set schedule). Huge speedup for repeated aggregations
- [ ] B. No view
...
- [ ] A. Use BI Engine (in-memory cache): reserve 50GB BI Engine. Vertex AI BI/ Looker dashboards use BI Engine: first query (computes), stores in RAM. Subsequent queries hit cache (instant). No query latency. Cost: extra monthly fee for BI Engine reservation
- [ ] B. No acceleration
- [ ] C. Larger slots
...
- [ ] A. Create authorized view: (1) SELECT customer_id, name, location FROM sensitive_table (hide salary), (2) GRANT SELECT on view to partner's project. Partner queries view (sees only 3 columns). View acts as security gate (data access control)
- [ ] B. Share raw table
- [ ] C. No sharing
- [ ] D. Manual filtering
...
- [ ] A. Time travel: SELECT * FROM `project.dataset.table`@-7200000 (2 hours ago, in milliseconds). BigQuery stores table snapshots (default 7 days retention). Restore deleted data from snapshot. Or use TIME TRAVEL in CREATE TABLE AS SELECT to restore specific time
- [ ] B. Data lost forever
- [ ] C. No snapshots
...
- [ ] A. Incremental load: (1) track max timestamp of last load, (2) load only WHERE timestamp > last_max, (3) append to table (upsert on unique ID if needed). Dataflow template: Cloud Storage → BigQuery (incremental). Reduces load time to <1 min. Standard pattern for streaming/batch ETL
...
- [ ] A. Not safe with string concatenation. Use parameterized queries: DECLARE user_id INT64 = @user_id; SELECT * FROM users WHERE user_id = user_id. BigQuery placeholders (@param) prevent SQL injection. Always use parameters for user input
...
- [ ] A. Federated query: CREATE EXTERNAL TABLE ext AS (SELECT * FROM `gs://bucket/data.parquet` FORMAT='PARQUET'). Then: SELECT * FROM bq_table JOIN ext ON bq_table.id = ext.id. BigQuery handles joining external + BQ tables. External queries scan Cloud Storage, slower/costlier than BQ native (but possible)
- [ ] B. Separate queries
- [ ] C. No federation
...
- [ ] A. Audit queries (Cloud Logging → find heavy queries). (1) Partition/cluster tables (reduce scan), (2) use materialized views (cache aggregations), (3) use BI Engine (dashboard cache), (4) reserve slots (cheaper bulk compute), (5) archive old data (delete or move to cold storage). Target: $2K/month (60% savings)
- [ ] B. No optimization
- [ ] C. Accept $5K
- [ ] D. Delete tables
...
- [ ] A. Use Pub/Sub → BigQuery direct write (no Dataflow): Pub/Sub writes directly to BigQuery (streaming inserts). Latency: <10 sec. OR: Pub/Sub → Dataflow → BigQuery (for transforms). Direct write is simpler (no pipeline), lower cost. Choose by complexity (transforms needed?)
- [ ] B. Dataflow required
...
- [ ] A. Column-level security: GRANT roles/analyticViewer on column with DATA_MASKING policy. Masking rules: HASH for PII, NULL for sensitive columns. Analytics team queries, sees only hashed/masked data. Big Query automatically applies masking per user role. Data governance via IAM
- [ ] B. No masking
...
- [ ] A. Dashboard in Looker/Datalab: (1) prediction volume (trend line), (2) accuracy/precision/recall (daily, weekly rolling average, alert if drop >2%), (3) latency histogram, (4) model predictions vs ground truth comparison (once labels arrive). Use Cloud Logging + Pub/Sub for real-time metric ingestion. Alert email if threshold breached
- [ ] B. No monitoring
...
- [ ] A. Split users: 50% use v1 (control), 50% use v2 (treatment). Run 2 weeks. Collect metrics: CTR, conversions, latency. Statistical test (t-test): is v2 CTR significantly >v1? If p-value <0.05, v2 wins; upgrade all. Use Vertex AI experiments feature to track, or Analytics tool (e.g., Firebase A/B testing)
- [ ] B. Deploy v2 to 100%
- [ ] C. No A/B testing
...
- [ ] A. Use Vertex AI Feature Store: (1) register features with ownership (team, SLA, schema), (2) centralized feature repository (single source of truth), (3) versioning (feature v1, v2), (4) governance (who can use? PII access logs), (5) point-in-time correctness (time-travel to get features as of training date). Teams maintain their features; others consume consistently
- [ ] B. No feature store
...
- [ ] A. Model explanation: (1) Explainable AI (SHAP values, integrated gradients) shows which features contributed to rejection (e.g., debt_ratio=0.8, income=0.1), (2) counterfactual: "If income was $60K (vs actual $40K), likely approved", (3) document model card (training data, fairness evaluation, limitations). Provide to customer + regulator
...
- [ ] A. Audit trail in Vertex ML Metadata: (1) training pipeline execution ID, (2) training data artifact (dataset version, time range), (3) training code version (Git commit), (4) evaluation metrics, (5) approval record (who, when). Reproduce: re-run pipeline with same inputs (deterministic if seeds fixed). All logged automatically
- [ ] B. No audit trail
- [ ] C. Lost history
- [ ] D. Manual notes
...
- [ ] A. Governance framework: (1) model registry (all models tracked, versioned, documented), (2) approval workflow (model passes tests before production), (3) ownership (each model has owner, SLA), (4) monitoring (all models monitored for drift, performance), (5) escalation (if model fails SLA, alert owner, revert if needed), (6) audit (all changes logged). Use Vertex AI Model Registry + Pipelines + custom CRUD API
- [ ] B. No governance
...
- [ ] A. Use incremental/online learning to update model weights on streaming new data (rather than full retraining)
- [ ] B. Retrain more frequently (e.g., daily) if the domain is fast-changing
- [ ] C. Never retrain; staleness is unavoidable
- [ ] D. Monitor model age (time since last training); alert if > threshold
...
- [ ] A. Model version and timestamp (for traceability)
- [ ] B. Prediction confidence/probability (for understanding certainty)
- [ ] C. Actual outcome (if available, for continuous monitoring)
- [ ] D. User ID of the decision-maker (if human-in-the-loop); helps audit override patterns
...
- [ ] B. Investigate the edge cases where v2 fails; if they're unacceptable, halt or optimize v2; continue canary at 5% while resolving issues
- [ ] C. Rollback to v1; latency increase is unacceptable
- [ ] D. Implement SLO thresholds: promote only if latency < 120ms and error rate < 2%; adjust canary traffic based on metrics
...
- [ ] A. Prediction divergence: how often do v1 and v2 disagree (e.g., AUC rank difference)?
- [ ] B. Latency and resource usage: does v2 meet performance SLOs before traffic is switched?
- [ ] C. Edge case detection: identify request types where v2 behaves unexpectedly
- [ ] D. User satisfaction (if surveys are available): does v2 impact user experience before serving it?
...
- [ ] A. Use power analysis calculators; for 1% absolute improvement (5% â 6%), roughly 4,000â5,000 samples per variant are needed
...
- [ ] D. Run longer than the minimum sample size to account for temporal effects (day-of-week, seasonality)
...
- [ ] A. Yes; rollback to v1 to restore service quickly
- [ ] B. While rolling back, start a post-mortem: investigate training bug, fix it, retrain on corrected data
- [ ] C. Don't rollback; let users suffer until v3 is ready
- [ ] D. Implement automated rollback triggers based on error rate spikes or performance degradation
...
- [ ] A. Benefits: catches mistakes, ensures accountability; Drawbacks: slower deployment, bottleneck on approvers
- [ ] B. Implement automated checks (data validation, performance thresholds) to reduce manual review burden
- [ ] C. Use Vertex AI Pipelines with approval gates (conditional execution) to enforce workflows
...
- [ ] A. Schema validation: columns match expected types and presence
- [ ] B. Statistical checks: distribution shifts, outliers, missing value rates
- [ ] C. Business logic checks: values within expected ranges (e.g., age 0â120)
- [ ] D. Freshness checks: data arrives on schedule; alerts if delays occur
...
- [ ] B. Context-dependent: static is more controlled and reproducible; dynamic adapts faster to new patterns but is harder to debug and may suffer from catastrophic forgetting
- [ ] C. Dynamic is always better; continuously learns from new data
- [ ] D. Use static training for baseline; add online learning components if drift is significant
...
- [ ] A. Use Vertex AI Feature Store API in the pipeline to fetch features with `snapshot_time`; log feature versions in Vertex ML Metadata
- [ ] B. Create a BigQuery view that queries the Feature Store's offline tables; train on the view; BigQuery handles versioning
- [ ] C. Export all features to CSV; load manually in the pipeline
- [ ] D. Document feature versions in the model's training metadata; enables reproducibility
...
- [ ] A. Checkpoint frequently (every 2 hours); resume from the latest checkpoint instead of restarting
- [ ] B. Use `restart_job_on_worker_restart=true` in Vertex Training configuration for fault tolerance
- [ ] C. Rerun the entire job; checkpoints add overhead
- [ ] D. Implement adaptive checkpointing: save more frequently when close to quota limits
...
- [ ] B. Not necessarily; if performance remains acceptable, drift is not critical; monitor performance as the primary signal and use drift as a secondary indicator
- [ ] C. Investigate why performance is stable despite drift: is the model robust to this drift, or is the test set unrepresentative?
- [ ] D. Use a hybrid approach: retrain if performance degrades OR drift exceeds a threshold (chosen to be conservative)
...
- [ ] A. Use Cloud Trace to profile each component; identify which is the bottleneck (e.g., feature retrieval = 300ms)
- [ ] B. Log component input/output sizes and durations to Cloud Logging; query logs to find slow steps
- [ ] C. Use distributed tracing (OpenTelemetry) to correlate latency across components
...
- [ ] A. Git commit hash of training code; container image digest; training dataset version (BigQuery table snapshot or dataset checksum)
- [ ] B. Hyperparameters (learning rate, batch size, num_epochs) used in training
- [ ] C. Random seed and TensorFlow version; enables deterministic reproduction
- [ ] D. Full training pipeline configuration and execution time; logs from training
...
- [ ] A. Use preemptible VMs for training (cheaper, but can be interrupted)
- [ ] B. Batch predictions offline when possible; use online serving only for latency-sensitive queries
- [ ] C. Archive old training data to Cloud Storage (colder tiers); keep only recent data in BigQuery
- [ ] D. Right-size model (smaller models = cheaper serving); use knowledge distillation to compress
...
- [ ] A. Cache frequently-used features locally (Redis); use cache on Feature Store outage
- [ ] B. Implement feature fallbacks: use stale features or pre-computed averages if real-time features are unavailable
- [ ] C. Make Feature Store calls optional; if they fail, skip features and serve predictions anyway
- [ ] D. Monitor Feature Store health; proactively fail over to read replicas or alternative data sources
...
- [ ] A. Log input features and model version; use post-hoc explanations (SHAP, LIME) on-demand for a specific prediction
- [ ] B. Pre-compute feature importance during training; include in serving (faster than on-demand)
- [ ] C. Always explain every prediction; compute SHAP values for all 100K daily predictions in real-time
- [ ] D. Combine global explanations (feature importance) with instance-level explanations (why this specific prediction?)
...
- [ ] A. Alert the team (always automate monitoring)
- [ ] B. Rollback to previous model (always automate; this is critical)
- [ ] C. Trigger data quality checks (safe; provides diagnostic info)
- [ ] D. Start retraining job (acceptable if configured with guardrails; can also alert for manual approval)
...
- [ ] A. Trade-off: transparent models are inherently explainable but may have lower performance; black-box + post-hoc explanations offer better performance but explanations may not be fully trustworthy
- [ ] B. Always use transparent models; explainability is non-negotiable
- [ ] C. Use the approach that meets business requirements: if interpretability is critical, use transparent; if performance is critical, use black-box + post-hoc
- [ ] D. Implement both; use transparent models when possible, but allow black-box models when justified
...
- [ ] A. Unit tests for data preprocessing (ensure transformations are correct)
- [ ] B. Integration tests for model inference (ensure model + preprocessing work together)
- [ ] C. Contract tests (ensure output schema matches expectations)
- [ ] D. Performance regression tests (ensure model inference latency doesn't degrade unexpectedly)
...
- [ ] B. Both are important, but indicate different issues: skew suggests training-serving misalignment (data pipeline bugs); drift suggests model aging (needs retraining); prioritize based on performance impact
- [ ] C. Drift is more urgent; skew is expected in production
- [ ] D. Investigate root cause: is skew due to a data pipeline bug (repairable) or legitimate distribution change (requires retraining)?
...
- [ ] A. Yes; increased feature importance indicates the model's decision-making has changed; investigate why
- [ ] B. This could signal concept drift (the relationship between features and labels has changed)
- [ ] C. Ignore; feature importance naturally varies with data
- [ ] D. Check if time-based features are predictive due to external events (e.g., seasonal sales); ensure this isn't spurious
...
- [ ] A. Increase thresholds to reduce alert fatigue (e.g., AUC drop >15%, drift > 1.5); monitor via dashboard (not alerts) for trending issues
- [ ] B. Lower thresholds to catch all issues; tolerate false alarms
- [ ] C. Remove monitoring; it generates too many alerts
- [ ] D. Use statistical significance testing (e.g., p-value < 0.05) instead of raw thresholds; adjust alert windows (daily vs. weekly)
...
- [ ] A. Yes; this is a data quality issue; investigate the data pipeline (bug in parsing or validation)
- [ ] B. Use TFDV (TensorFlow Data Validation) to define schema constraints and catch invalid values before they reach the model
- [ ] C. Implement automated alerts for schema violations (unexpected data types, ranges)
...
- [ ] A. Model output may not align with business goals; re-examine feature engineering or loss function; optimize for business metrics, not just classification metrics
- [ ] B. Business metrics are unreliable; trust ML metrics instead
- [ ] C. External factors (market conditions, competitors) may affect business metrics independent of model quality
- [ ] D. Implement monitoring for both ML metrics and business metrics; correlate them to validate model value
...
- [ ] B. Update baselines periodically (e.g., monthly or when data significantly changes) to reflect current expectations; use versioned baselines to track evolution
- [ ] C. Use a separate baseline per day-of-week to account for weekly patterns
- [ ] D. Allow multiple baselines: training baseline (for initial validation), recent baseline (for trend detection)
...
- [ ] A. Monitor for anomalous input patterns (outliers in feature space); use methods like LOF (Local Outlier Factor) or isolation forests
- [ ] B. Track prediction changes on deliberately perturbed inputs (adversarial robustness testing); large changes indicate vulnerability
- [ ] C. Assume models are robust; attacks are unlikely
- [ ] D. Implement input validation (range checks, schema validation) to reject suspicious inputs
...
- [ ] A. Focus on feature retrieval; it's the bottleneck; cache features or pre-compute
- [ ] B. Profile inference latency by layer; may find slow layers worth optimizing or quantizing
- [ ] C. Use resource monitoring (CPU/memory) to identify if the system is CPU-bound, memory-bound, or I/O-bound
...
- [ ] A. Retrain if AUC drop is statistically significant and persists; use proxy labels (e.g., user feedback) for faster validation before ground truth arrives
- [ ] B. Combine signals: AUC drop + feature drift together suggest retraining is warranted; any single signal alone might be noise
- [ ] C. Wait for ground truth labels (2 weeks) before deciding; cannot act without them
- [ ] D. Retrain on fresh data using the same hyperparameters; if performance improves, promote the new model
...
- [ ] A. Gradual degradation: retrain on recent data; this is expected model aging
- [ ] B. Sudden drop: immediately investigate; check for data pipeline bugs, label leakage, or external disruptions
- [ ] C. For sudden drops, consider automatic rollback to previous model; for gradual degradation, automatic retraining is acceptable
...
- [ ] A. Overfitting on training data; generalization gap between train and validation; additional degradation in production suggests data/concept drift
- [ ] B. Model is broken; all metrics should be equal
- [ ] C. Retrain on recent data; the validation set is stale (1 month old)
- [ ] D. Use production data (if labels become available) to update the validation set; re-validate the model
...
- [ ] A. Potentially yes; Group B has lower recall (may miss important cases for that group); investigate whether this is acceptable or indicates bias
- [ ] B. No; precision is similar, so the model is fair
- [ ] C. Check if the differences are due to data quality (Group B may have fewer training examples or noisier labels)
- [ ] D. Define fairness requirements for this use case (equal recall, equal precision, or demographic parity?) and monitor accordingly
...
- [ ] A. Use Cloud Monitoring to capture Vertex AI Model Monitoring metrics; configure alerting policies to trigger Slack/PagerDuty notifications
- [ ] B. Use Cloud Logging to log monitoring results; downstream systems consume logs and route alerts
- [ ] C. Integrate with incident management tools (PagerDuty, Opsgenie) for on-call escalation on critical issues
...
- [ ] B. No; expected seasonal patterns should not trigger alerts; configure monitoring to account for seasonality (e.g., use Q4-specific baselines)
- [ ] C. Implement seasonal baselines: separate baselines for Q1âQ4; compare production data to the corresponding seasonal baseline
- [ ] D. Monitor for anomalous seasonality (e.g., Q4 spike is smaller than historical average by 2 standard deviations); this may indicate real problems
...
- [ ] A. Feed monitoring insights to the data science team; prioritize retraining if drift is severe
- [ ] B. Log monitoring alerts in a ticketing system (Jira); create backlog items for investigation
- [ ] C. Automate retraining when specific thresholds are crossed (e.g., drift > 1.5)
- [ ] D. Use A/B testing to validate that retraining actually improves business metrics before full rollout
...
- [ ] A. Batch predictions via Dataflow/BigQuery for use case 1; online serving via Vertex AI Endpoints for use case 2
- [ ] B. Use online serving for both; batch is outdated
- [ ] C. Use batch for both; simpler infrastructure
- [ ] D. Batch is cost-effective for high volume, low freshness tolerance; online is expensive but provides low latency
...
- [ ] A. `min_replica_count=1` (handle off-peak); `max_replica_count=100` (handle peak)
- [ ] B. `target_cpu_utilization=0.7` or `target_request_count_per_replica` to decide scaling up
- [ ] C. Fixed replicas=50; autoscaling is overhead
- [ ] D. Monitor scaling latency; set scale-up threshold conservatively to avoid serving degradation during ramp-up
...
- [ ] A. T4 is cheapest; test latency first; if <50ms, use T4
- [ ] B. V100 and A100 are more expensive but faster; use if T4 latency exceeds SLO
- [ ] C. Profile model with different batch sizes (1, 4, 8) on each GPU type; choose based on throughput-per-dollar
...
- [ ] A. Use Vertex AI Endpoints' traffic splitting feature to route requests by percentage
- [ ] B. Tag models in the Vertex AI Model Registry with environment labels (v1:prod, v2:canary)
- [ ] C. Maintain model lineage: v2 was trained on dataset_v1.2, hyperparameters X, with performance metrics Y
- [ ] D. After full promotion, retain v1 as a rollback version for at least 30 days
...
- [ ] A. Cloud Run is suitable for lightweight models with low latency; cheaper than Vertex Endpoints for occasional traffic
- [ ] B. Vertex Endpoints is always better; Cloud Run is too simple
- [ ] C. Evaluate: Cloud Run has ~100ms cold-start overhead; if your model latency is 10ms, total latency â 110ms (may exceed SLO)
- [ ] D. Use Vertex Endpoints if latency SLO is tight (<100ms); use Cloud Run for looser SLOs
...
- [ ] A. Convert to TensorFlow Lite format; it's ~75% smaller (150MB)
- [ ] B. Apply quantization (INT8): reduces size further and speeds up inference on mobile
- [ ] C. Run inference on the backend; send predictions to the device (costs per-request, higher latency)
- [ ] D. Include both: TFLite + quantization + pruning if latency is still problematic
...
- [ ] A. Cache frequently accessed features in Redis; use Feature Store as a fallback
- [ ] B. Pre-fetch features in batch mode before serving (if applicable); reduces per-request latency
- [ ] C. Use an in-process feature cache (updated periodically); trade memory for latency
- [ ] D. Optimize Feature Store queries: add indexes, use point lookups instead of range queries
...
- [ ] A. Use Cloud Load Balancer with geo-routing; routes to nearest region
- [ ] B. Use Cloud CDN in front of endpoints; caches responses, reduces latency
- [ ] C. Implement custom routing logic: client determines nearest endpoint and calls directly
...
- [ ] A. Use Vertex Endpoints' traffic splitting; route requests probabilistically (5% v2, 95% v1)
- [ ] B. Log request IDs and variant (v1 or v2) to BigQuery; enable statistical analysis post-hoc
- [ ] C. Deterministic routing: route user_id % 20 == 0 to v2; ensures consistency
- [ ] D. Analyze metrics: latency, errors, business outcomes; decide to promote or rollback after 1 week
...
- [ ] A. Define a request schema (JSON Schema or Protocol Buffers); validate before inference
- [ ] B. Return error responses for invalid inputs (HTTP 400); log for debugging
- [ ] C. Allow the model to handle invalid inputs gracefully; it's robust
- [ ] D. Use Vertex AI Endpoints' built-in request validation features (if available)
...
- [ ] A. Redact sensitive fields before logging; keep only non-sensitive features (e.g., age, income)
- [ ] B. Use separate audit logs with restricted access (encryption, IAM); limited to authorized personnel
- [ ] C. Hash sensitive fields; log hashes instead of raw values (enables debugging without exposing data)
...
- [ ] A. Pre-compute explanations offline for common requests; cache and serve cached explanations
- [ ] B. Serve predictions fast (<100ms); generate explanations asynchronously in the background; return to user later (e.g., via email)
- [ ] C. Use approximation methods (LIME instead of SHAP) for faster computation
...
- [ ] A. Stream logs to BigQuery for efficient querying; partition by date and model version
- [ ] B. Use Cloud Logging with appropriate retention policies (e.g., delete logs after 30 days)
- [ ] C. Sample logs (e.g., 10% of requests) for cost savings; ensure sampling is representative
- [ ] D. Compress logs; use columnar formats (Parquet) for storage efficiency
...
- [ ] A. Use cached features (from previous requests); serve predictions with slightly stale features
- [ ] B. Use default feature values (pre-computed averages) if live features are unavailable
- [ ] C. Return a timeout error to users; do not serve predictions with degraded inputs
- [ ] D. Automatically switch to a simpler, faster model that doesn't require the unavailable dependency
...
- [ ] A. Create a Dockerfile with base image (Python 3.9) and custom dependencies; push to Container Registry
- [ ] B. Build custom container image; specify in the model deployment config; Vertex deploys the custom container
- [ ] C. Modify Vertex's built-in container (not possible; use custom container instead)
- [ ] D. Test the container locally (Docker run) before deploying to Vertex
...
- [ ] A. Use a request queue (e.g., Cloud Tasks or Pub/Sub); batch requests before inference
- [ ] B. Implement request buffering: wait up to 100ms for requests to accumulate before inferencing (trade latency for throughput)
- [ ] C. Batch all requests; no single-request serving
- [ ] D. Adaptive batching: vary batch size based on queue depth (larger batch if queue is deep)
...
- [ ] A. Investigate why v2 is slower; may be resolvable (e.g., optimize dependencies)
- [ ] B. Halt rollout; latency increase is unacceptable
- [ ] C. If 20ms is acceptable, continue rollout; if not, optimize v2 before proceeding
- [ ] D. Establish latency SLO; only promote if v2 meets SLO
...
- [ ] A. Use rolling updates: gradually move traffic from old to new model; no requests are dropped
- [ ] B. Stop the endpoint, update, restart (results in downtime)
- [ ] C. Use Vertex Endpoints' update mechanism; it handles in-place upgrades without downtime
- [ ] D. Implement health checks; only route traffic to healthy replicas during update
...
- [ ] A. Cache based on input features (hashed feature vector) â prediction; use Redis for fast lookups
- [ ] B. Set cache TTL (time-to-live) based on domain (e.g., 1 hour for recommendations; stale is acceptable)
- [ ] C. Cache all predictions forever; they never change
- [ ] D. Invalidate cache when model version changes (v1 predictions should not be served after v2 is deployed)
...
- [ ] A. Deploy all models to the same Vertex Endpoint; use a routing layer to direct requests to the appropriate model
- [ ] B. Assign each model to a route (e.g., `/predict/ctr`, `/predict/revenue`, `/predict/affinity`); clients specify which model to call
- [ ] C. Require clients to choose a model before creating an endpoint (static routing)
- [ ] D. Use a meta-model to decide which model to use (if applicable); route dynamically
...
- [ ] B. Random search: sample 50â100 random configurations; often finds good hyperparameters efficiently
- [ ] C. Bayesian search (e.g., Vertex AI Vizier): models the objective function; prioritizes promising regions; typically outperforms random
- [ ] D. Start with random; if promising patterns emerge, use Bayesian for refinement
...
- [ ] A. Suggest hyperparameter configurations based on Bayesian optimization; learn from previous trials to improve suggestions
- [ ] B. You run training for each suggested configuration; report results (AUC) back to Vizier
- [ ] C. Vizier runs training automatically; you only need to define the search space
- [ ] D. Vizier outputs the top-N configurations; you select the best for production
...
- [ ] A. Yes; stop training if validation loss doesn't improve for 3 consecutive epochs; redeploy compute to new trials
- [ ] B. No; all trials should run the full duration for fairness
- [ ] C. Early stopping frees up compute; allows more trials in the same budget
- [ ] D. Be careful: aggressive early stopping may discard configurations that improve later; use moderate patience (5â10 epochs)
...
- [ ] A. Focus future tuning efforts on high-importance hyperparameters (learning_rate); less critical ones can be held fixed
- [ ] B. For deployment, establish sensitivity: if learning_rate changes by 10%, does AUC change significantly? If so, keep it fixed
- [ ] C. Importance scores are final; don't retune high-importance hyperparameters
- [ ] D. Different models/datasets may have different importance scores; repeat analysis if domain shifts
...
- [ ] A. Yes; use a learning rate schedule for each trial; report final AUC to Vizier
- [ ] B. Apply the same schedule (e.g., warmup for 10% of steps, linear decay) to all trials for fairness
- [ ] C. Let Vizier suggest learning rate schedules as hyperparameters (too many combinations)
- [ ] D. If using a fixed schedule, ensure it doesn't confound Vizier's tuning (the schedule itself becomes a hidden hyperparameter)
...
- [ ] A. Allocate more tuning budget to Boosted Trees (more hyperparameters = larger search space)
- [ ] B. Use importance analysis post-hoc; some hyperparameters may be less critical (can reduce search space)
- [ ] C. Allocate equal budget; the models should be treated fairly
- [ ] D. Tune the simpler model (Linear) first; baseline results help calibrate the search for the more complex model
...
- [ ] A. Hyperparameters from the original task are a reasonable starting point; may still be optimal for fine-tuning
- [ ] B. The new task may require different hyperparameters; tune on the new task to be sure
- [ ] C. Always retune from scratch; prior hyperparameters are task-specific
- [ ] D. Perform ablation: train with original hyperparameters vs. newly tuned; compare to quantify the benefit
...
- [ ] A. Yes; with larger effective batch size (4Ã32=128), learning_rate may need to increase (larger batch = noisier gradients)
- [ ] B. No; hyperparameters should be independent of parallelism
- [ ] C. Use batch size scaling heuristic: lr_new â lr_old Ã (new_batch_size / old_batch_size)^0.5
- [ ] D. Validate with a quick tune: test a few learning rates on the distributed setup; pick the best
...
- [ ] A. Depends on the use case: if interpretability is critical (e.g., lending), sacrifice 2% accuracy for simplicity
- [ ] B. Use the complex model + post-hoc explanations (SHAP) for interpretability
- [ ] C. Always choose the highest-accuracy model; interpretability is a luxury
- [ ] D. Consider business cost: does 2% accuracy loss translate to significant revenue loss? If not, prefer simplicity
...
- [ ] A. Yes; ensembles of diverse models often outperform single models; the top-5 are likely diverse (different hyperparameters)
- [ ] B. No; ensembles don't improve if models are similar
- [ ] C. Validate: train ensemble on the top-5; test on held-out data; compare to best individual model
- [ ] D. Ensemble is more expensive to serve (5Ã inference calls); weigh cost vs. performance gain
...
- [ ] A. Prompt engineering first; it's fast and free; if it achieves acceptable performance, use it
- [ ] B. Always fine-tune; it's more robust than prompt engineering
- [ ] C. If prompt engineering is insufficient (<70% accuracy), proceed to fine-tuning
- [ ] D. Fine-tuning on 1000 examples may cost $100â1000 and time; weigh against prompt engineering ROI
...
- [ ] A. Vector embeddings: embed documents and queries into a common space; retrieve similar documents via vector similarity
- [ ] B. Retriever (e.g., Chroma, Pinecone): efficient data structure for nearest-neighbor search
- [ ] C. LLM (e.g., Vertex AI Generative API): reads augmented prompt and generates response
- [ ] D. Ranking/reranking: optionally re-rank retrieved documents by relevance before passing to LLM
...
- [ ] A. Fixed-size chunks (e.g., 512 tokens) with overlap (e.g., 50-token overlap) to maintain context
- [ ] B. Semantic chunking: split at logical boundaries (e.g., section breaks, paragraph ends)
- [ ] C. Query-based chunking: chunk differently based on expected query patterns
...
- [ ] A. Embedding quality: higher quality captures semantic meaning better; Text Embedding API is often highest quality but slower/more expensive
- [ ] B. Dimensionality: higher-dim embeddings are more expressive but slower to search and require more storage
- [ ] C. Latency: if retrieval must be <100ms, lightweight embeddings may be required
- [ ] D. Domain-specificity: if your domain is specialized (e.g., biomedical), fine-tuned embeddings may outperform general-purpose
...
- [ ] A. Retrieval metrics: NDCG (rank-weighted recall), Recall@k (is the relevant doc in top-k?), Precision@k
- [ ] B. Generation metrics: BLEU, ROUGE (lexical overlap with ground-truth answers); may be insufficient (multiple correct answers exist)
- [ ] C. End-to-end metrics: user satisfaction (Likert scale), correctness (human annotation)
- [ ] D. Latency and cost: retrieval + LLM inference latency; embedding and LLM API costs
...
- [ ] A. LoRA (Low-Rank Adaptation): add trainable low-rank matrices; freeze the base model; drastically reduces memory (requires only â1GB)
- [ ] B. QLoRA: quantize the model (INT4) + LoRA; even more memory-efficient (â6GB for 7B model)
- [ ] C. Prompt engineering instead of fine-tuning (not an alternative to fine-tuning, but a workaround)
- [ ] D. Use Vertex AI's managed fine-tuning; infrastructure is abstracted; no need to manage GPU memory
...
- [ ] B. Few-shot typically achieves higher accuracy; examples teach the model the task
- [ ] C. Few-shot uses more tokens (higher cost); trade accuracy for cost
- [ ] D. Start with zero-shot; if accuracy is insufficient, upgrade to few-shot
...
- [ ] A. LLM generates intermediate steps; more transparent reasoning
- [ ] B. Complex reasoning tasks (multi-step math, logic) often improve with step-by-step prompting
- [ ] C. Chain-of-thought always improves accuracy; use it for all tasks
- [ ] D. Trade-off: more tokens used (higher cost); slower generation
...
- [ ] A. Use citation mechanisms: require the LLM to cite which retrieved document supports each claim
- [ ] B. Prompt engineering: instruct the model to only use information from the provided context
- [ ] C. Constrained decoding: limit LLM outputs to facts present in retrieved documents (more complex but effective)
- [ ] D. Evaluate with ground truth; fine-tune on examples where the model hallucinated
...
- [ ] A. Maintain a rolling context window: keep the last N messages; include them in each request to the LLM
- [ ] B. Summarization: periodically summarize old messages into a compact summary; include summary + recent messages
- [ ] C. Each message is independent; context windows are not needed
- [ ] D. Be aware of token limits: long conversations exceed model context windows (e.g., 4K tokens); truncate or summarize
...
- [ ] A. Gemini 1.5's large context is ideal for long legal documents; in-context learning without fine-tuning
- [ ] B. Fine-tuning on legal data may improve accuracy further; combine with Gemini for best results
- [ ] C. GPT-4 via API is cheaper than Vertex AI (often incorrect; costs vary)
- [ ] D. Evaluate total cost: API calls Ã frequency vs. fine-tuning cost + reduced latency
...
- [ ] A. Temperature tuning: lower temperature (e.g., 0.3 instead of 1.0) reduces randomness; more deterministic, less creative outputs
- [ ] B. Retrieval-augmented generation (RAG): ground outputs in factual documents
- [ ] C. Prompt engineering: instruct the model to say "I don't know" if unsure, rather than guess
- [ ] D. Fine-tuning on high-quality, factual data (not internet scraped data with misinformation)
...
- [ ] A. Nearest neighbor search: find top-k most similar vectors to a query vector (efficient with ANN indexing)
- [ ] B. Semantic filtering: combine vector similarity with metadata filters (e.g., "find similar documents from 2024")
- [ ] C. Full-text search on vector embeddings (defeats the purpose; embeddings don't support text search directly)
- [ ] D. Hybrid search: combine vector similarity with keyword-based search for better results
...
- [ ] A. Check for duplicates (remove); deduplicate reduces data efficiency
- [ ] B. Validate prompt-response consistency (do responses actually answer the prompts?)
- [ ] C. Check for bias (are all examples representative of your use case, or skewed?)
- [ ] D. Balance dataset: if most examples are easy, include harder examples to improve robustness
...
- [ ] A. Batch documents: embed 100 documents in a single API call; reduces overhead; roughly 1M tokens (1K/doc Ã 1M docs, with less overhead)
- [ ] B. Batch size doesn't matter; API pricing is per-document
- [ ] C. Calculate cost difference: 1.5M vs. 1M tokens Ã $0.0001 / 1K = significant savings
- [ ] D. Use Vertex AI Batch Prediction API (if available) for large-scale embedding; may offer volume discounts
...
- [ ] A. Cross-encoder ranking: rerank retrieved documents; pass only top-3 to LLM (higher quality, but reduces context)
- [ ] B. Threshold filtering: discard documents with relevance score <0.5; ensures retrieved documents are relevant
- [ ] C. Use all top-10; LLM will filter irrelevant documents (often doesn't; LLM gets confused by noise)
- [ ] D. Multi-stage ranking: semantic + keyword + domain-specific signals for robust ranking
...
- [ ] A. Input validation: filter prompts for suspicious patterns (e.g., "generate credential", "bypass security")
- [ ] B. Output validation: filter LLM responses for sensitive data (passwords, PII, API keys)
- [ ] C. Fine-tune on safe examples; instruct tuning to refuse harmful requests
- [ ] D. Use specialized guardrail APIs (e.g., Google Cloud's Guardrails for GenAI)
...
- [ ] A. Input cost: 3.65M queries Ã 200 tokens Ã $0.001 / 1K = $730
- [ ] B. Output cost: 3.65M queries Ã 100 tokens Ã $0.003 / 1K = $1,095
- [ ] C. Total: $730 + $1,095 = $1,825 / year (excluding infrastructure)
- [ ] D. Add infrastructure costs (embeddings, vector DB, storage) to get total cost; may double or triple the LLM cost
...
- [ ] A. Streaming reduces perceived latency; users see results immediately
- [ ] B. Streaming requires client-side buffering; servers push tokens as they arrive
- [ ] C. Streaming is always faster; no trade-offs
- [ ] D. For RAG systems: stream the response after retrieval; no speed gain from retrieval (already async)
...
- [ ] A. Smaller model is faster and cheaper to serve; trades some accuracy for efficiency
- [ ] B. Distillation requires generating training data (100K examples from large model); adds cost
- [ ] C. Distilled models always match the teacher's accuracy; no degradation
- [ ] D. Distillation works well if the task is well-covered by training data; custom tasks may not distill as well
...
- [ ] A. Domain-specific terminology: fine-tuning teaches the model medical jargon and context
- [ ] B. Privacy and compliance: medical data is sensitive; ensure HIPAA compliance (e.g., deidentification, secure storage)
- [ ] C. Domain evaluation: use domain-specific metrics (e.g., medical accuracy, clinician review) not generic benchmarks
- [ ] D. Continuous updates: medical knowledge evolves; periodically retrain on new evidence
...
- [ ] A. Version prompts in Git; tag versions that go to production
- [ ] B. Store templates in a Prompt Management System (e.g., LangChain Hub); track A/B test results per version
- [ ] C. Document prompt changes: what changed, why, expected impact
- [ ] D. Test new prompts on a holdout set before production deployment (similar to model A/B testing)
...
- [ ] A. Sample 1K documents; have humans label them; compare human vs. LLM labels; estimate LLM accuracy
- [ ] B. Use agreement between multiple LLM models (ensemble voting) to identify uncertain predictions
- [ ] C. Fine-tune a classification model on LLM labels; evaluate on human-labeled holdout set
- [ ] D. Iteratively improve: use model predictions + human feedback to refine labels (active learning)
...
- [ ] A. Extract text from scanned PDFs (OCR replacement); Gemini reads the image and outputs text
- [ ] B. Classify documents by type (invoice, receipt, contract) by reading the image
- [ ] C. Summarize document content; Gemini understands both text and visual layout
...
- [ ] A. Use a robust system prompt; prioritize safety over flexibility
- [ ] B. Implement input validation; filter suspicious patterns (e.g., "system prompt", "execute", "ignore")
- [ ] C. Fine-tune on adversarial examples; teach the model to resist attacks
- [ ] D. Monitor for unusual patterns (e.g., unusual request volume, same user repeated attempts); rate-limit or block
...
- [ ] A. Training data bias: if training data underrepresents women or contains biased labels, the model learns biased patterns
- [ ] B. Algorithmic bias: some algorithms are inherently more biased (e.g., using gender-correlated features)
- [ ] C. Measurement bias: if evaluation metrics differ between groups (e.g., different thresholds), bias may be masked
- [ ] D. Representation bias: if test set underrepresents women, evaluation is unreliable
...
- [ ] B. No; the 10% gap violates the equal precision criterion
- [ ] C. But fairness has trade-offs; improving Group B precision may reduce overall model accuracy
- [ ] D. Choose a different fairness definition (e.g., equalized odds) if the chosen one is unachievable; document the choice
...
- [ ] A. SHAP: use for smaller models/datasets where speed is acceptable; provides theoretically grounded importance scores
- [ ] B. LIME: use for real-time serving; trades accuracy for speed; suitable for approximate explanations
- [ ] C. XRAI: use for image-based models; highlights salient image regions driving predictions
- [ ] D. Ensemble: pre-compute SHAP for offline analysis; use LIME for real-time explanations
...
- [ ] A. Saliency maps: highlight image regions the model focuses on; use Grad-CAM or SHAP
- [ ] B. Attention visualization: if the model uses attention layers, visualize which regions it attends to
- [ ] C. Counterfactual explanations: show what would need to change in the image for the model to change its decision
- [ ] D. Replace the model with a simpler, interpretable model (trade accuracy for interpretability)
...
- [ ] A. TCAV measures importance of human-defined concepts (e.g., "striped", "flying") to model predictions
- [ ] B. You provide examples of the concept (striped images); TCAV computes the concept's influence on the model
- [ ] C. TCAV is model-agnostic; works on any model without modification
- [ ] D. TCAV helps debug model behavior: if an undesired concept is influential, you can address it (e.g., collect more diverse data)
...
- [ ] A. DP adds noise to gradients during training; provides a mathematical guarantee that individual records are protected
- [ ] B. DP has a privacy budget (epsilon, delta); tighter budgets (lower epsilon) mean more privacy but worse model accuracy
- [ ] C. DP is training-time noise injection; does not protect against model inversion attacks at inference (use additional safeguards)
- [ ] D. DP-SGD (Differentially Private Stochastic Gradient Descent) is an implementation; fit hyperparameters (noise scale, batch size) for your privacy requirements
...
- [ ] A. Raw data never leaves devices; central server only sees gradients (more private than centralized training)
- [ ] B. Gradients can leak information (membership inference attacks); combine FL with differential privacy for stronger privacy
- [ ] C. Communication cost is high; sending gradients for 1M-parameter models to 1000 devices incurs significant bandwidth
- [ ] D. FL works best for latency-tolerant applications (updates every few hours, not real-time)
...
- [ ] A. Rebalancing training data: oversample Group B or undersample Group A (changes class distribution)
- [ ] B. Fairness-aware learning: add fairness constraints to the loss function (e.g., equal precision across groups)
- [ ] C. Post-processing: adjust model outputs to meet fairness criteria (e.g., set decision threshold per group)
- [ ] D. Data augmentation: generate synthetic Group B samples to balance dataset
...
- [ ] A. Disaggregation reveals performance disparities that aggregate metrics hide
- [ ] B. A single metric (0.88) is misleading if it masks inequality across groups
- [ ] C. Business/ethical requirement: fairness regulations (e.g., GDPR Article 22) demand transparency per group
...
- [ ] A. Model details: architecture, training data, evaluation metrics
- [ ] B. Limitations: known failure modes, intended use cases, unsuitable applications
- [ ] C. Ethical considerations: bias evaluation, fairness metrics, disparities across groups
- [ ] D. Reproducibility: hyperparameters, code, datasets (or pointers to them)
...
- [ ] A. Carbon footprint: compute costs translate to CO2 emissions; consider using renewable energy sources
- [ ] B. Model efficiency: smaller models (distillation, pruning) reduce carbon footprint significantly
- [ ] C. Environmental impact is not a concern for ML practitioners
- [ ] D. Report training costs/energy usage in model cards; transparency enables accountability
...
- [ ] A. Explore-exploit: periodically recommend items outside the learned preference to break feedback loops
- [ ] B. Fair ranking: ensure recommendations include diversity (e.g., 30% underrepresented items)
- [ ] C. Weighted sampling: boost underrepresented items' weight during training
- [ ] D. Monitor recommendation diversity over time; alert if diversity decreases
...
- [ ] A. Context-dependent: in high-stakes applications (e.g., criminal justice), fairness may outweigh overall accuracy
- [ ] B. Negotiate trade-offs with stakeholders; fairness is not a technical problem alone
- [ ] C. Fairness always reduces accuracy; unachievable to have both
- [ ] D. Quantify trade-offs: present stakeholders with multiple options (fairness threshold vs. accuracy) and their costs
...
- [ ] A. Third-party audit: external team tests the model independently, reports findings
- [ ] B. Red-teaming: adversarial testing to find failure modes, biases, adversarial examples
- [ ] C. Benchmark testing: evaluate on publicly available datasets and compare to established baselines
- [ ] D. Continuous monitoring: after deployment, track fairness/accuracy metrics and alert if they degrade
...
- [ ] A. Feature importance: which features drove the denial decision?
- [ ] B. Rule-based explanations: if the model uses interpretable rules (e.g., decision tree), explain using the rules
- [ ] C. Counterfactual explanations: "If your income were $10K higher, you'd be approved"
...
- [ ] A. Informed consent: users should understand how their data is used (training model, profiling, etc.)
- [ ] B. Data minimization: collect only data necessary for the stated purpose
- [ ] C. Data deletion: users can request deletion (right to be forgotten); model retraining may be required
- [ ] D. Transparency: document data sources, retention periods, and uses
...
- [ ] A. Adversarial training: train on adversarial examples; model learns to resist attacks
- [ ] B. Certified defenses: use techniques that provide formal robustness guarantees
- [ ] C. Ensemble models: combine multiple models; adversarial examples may not fool all models simultaneously
- [ ] D. Input smoothing: add noise during inference; reduces vulnerability to carefully crafted attacks
...
- [ ] A. Explainability: judges must understand why the model flagged someone as high-risk
- [ ] B. Fairness auditing: ensure racial/demographic parity in predictions (historical biases in data are pervasive)
- [ ] C. Human-in-the-loop: model provides recommendations; humans make final decisions
- [ ] D. Regular audits: track disparities over time; retrain with corrected data if needed
...
- [ ] A. Stakes of decisions: high-stakes (medical) require interpretability; low-stakes (product recommendations) can use black-box
- [ ] B. Regulatory environment: GDPR/FCRA may mandate explainability; choose accordingly
- [ ] C. User trust: interpretable models may earn user trust better, even if less accurate
- [ ] D. Hybrid approach: use black-box model for predictions + post-hoc explanations (SHAP/LIME) to balance accuracy and interpretability
...
- [ ] A. Model review boards: technical and ethics review before production deployment
- [ ] B. Documentation requirements: model cards, bias audits, limitations documented for all models
- [ ] C. Training and education: all data scientists understand bias, fairness, privacy implications
- [ ] D. Incident response: establish procedures for responding to model failures or discovered biases
...
- [ ] A. Static: simpler to implement, reproducible, easier to debug; slower to adapt to rapid changes
- [ ] B. Dynamic: adapts faster to new patterns; harder to debug, risk of catastrophic forgetting
- [ ] C. Static often uses A/B testing for validation; dynamic uses online metrics
- [ ] D. Hybrid: static training weekly + dynamic fine-tuning for rapid adaptation
...
- [ ] A. Feature drift: input distributions changed; model may be extrapolating beyond training domain
- [ ] B. Model drift: weights have shifted (often due to retraining); can be positive or negative
- [ ] C. Concept drift: the underlying relationship changed (e.g., customer behavior changed due to market conditions); requires retraining on new data
- [ ] D. All three may occur together; diagnose root cause before intervention
...
- [ ] A. ExampleGen: data ingestion from various sources
- [ ] B. StatisticsGen: compute statistics on data; feed to TFDV for validation
- [ ] C. Transform: feature engineering using TensorFlow Transform (TFT)
- [ ] D. Trainer: model training; Tuner: hyperparameter tuning; Evaluator: model evaluation
...
- [ ] A. Cluster sizing: balance cost (larger cluster = more cost) vs. speed (more parallelism = faster)
- [ ] B. Auto-scaling: scale workers up/down based on job load; saves cost during idle periods
- [ ] C. Spot/Preemptible instances: cheaper, but can be interrupted; acceptable for fault-tolerant jobs
- [ ] D. Network bandwidth: 1TB transfer to/from cluster may be slow; consider colocation of data and compute
...
- [ ] A. Statistical tests: compare training and production data distributions; alert if they diverge significantly
- [ ] B. Schema validation: ensure production data schema matches training schema (same columns, types)
- [ ] C. Feature audit: log features used in production; compare against training features
- [ ] D. A/B testing: serve both training and production models; compare metrics to quantify skew impact
...
- [ ] A. Single source of truth: all models tracked in one place with versioning
- [ ] B. Lineage tracking: which data and code produced each model version
- [ ] C. Metadata: model type, performance metrics, training hyperparameters, associated pipelines
- [ ] D. Governance: approval workflows, deployment tracking, versioning history
...
- [ ] A. CI triggers: on every commit/PR, execute the pipeline
- [ ] B. Automated tests: unit tests for data preprocessing, model inference, etc.
- [ ] C. Model validation: enforce performance thresholds (e.g., AUC > 0.80) before approval
- [ ] D. Approval gates: human review required for high-risk changes; automated approval for low-risk
...
- [ ] A. Canary deployment: route 5% traffic initially; monitor; gradually increase if stable
- [ ] B. Blue-green deployment: maintain two production environments; switch traffic when ready
- [ ] C. Rolling deployment: gradually replace old models with new; no downtime
- [ ] D. Shadow deployment: new model runs alongside old; no user impact; compare predictions
...
- [ ] A. Load balancing: distribute requests across GPUs; Cloud Load Balancer routes traffic
- [ ] B. Request batching: aggregate requests before inference; improves GPU utilization
- [ ] C. Model sharding: split large model across multiple GPUs; enables larger models than fit on one GPU
- [ ] D. Caching: cache predictions for identical inputs; reduces redundant computation
...
- [ ] A. Immediate rollback: revert to previous stable model; minimize user impact
- [ ] B. Diagnostics: check input data quality, model artifacts, dependencies (Feature Store, etc.)
- [ ] C. Data quality alerts: if data issues detected, pause serving until fixed
- [ ] D. Automated fallback: switch to a secondary model (e.g., heuristic-based) while investigating
...
- [ ] A. Model inference: use smaller models (distillation), quantization, hardware accelerators (TPUs vs. GPUs)
- [ ] B. Data storage: archive old data, use colder storage tiers, deduplication
- [ ] C. Training: use preemptible VMs, distributed training with efficient communication
- [ ] D. Feature Store: cache features, reduce online vs. offline store writes
...
- [ ] A. Label efficiency: achieve target accuracy with fewer labels (50% less labeling for comparable accuracy)
- [ ] B. Targeted labeling: focus on hard/uncertain examples; they're most valuable for learning
- [ ] C. Iterative improvement: each labeling round improves the model; can stop early when performance plateaus
...
- [ ] A. Threshold tuning: adjust decision threshold to balance precision/recall for your use case
- [ ] B. Class weighting: up-weight the minority class during training
- [ ] C. Ensemble methods: combine multiple models; some may specialize in minority class
- [ ] D. Data augmentation: generate synthetic minority examples (SMOTE); improves representation
...
- [ ] A. Pre-compute offline: compute features nightly, cache in memory/database; serve from cache at inference time
- [ ] B. Real-time computation: compute features on-demand; acceptable if <30ms
- [ ] C. Hybrid: pre-compute batch features; compute only missing real-time features on-demand
- [ ] D. Bloom filters / approximate counts: trade accuracy for speed (acceptable for non-critical features)
...
- [ ] A. Demographic parity: monitor accuracy, precision, recall per demographic group; alert if disparity exceeds threshold
- [ ] B. Equalized odds: monitor true positive rate and false positive rate per group; alert on disparity
- [ ] C. Causal fairness: monitor if model decisions are independent of protected attributes (advanced, requires causal inference)
- [ ] D. Threshold effects: for binary predictions, monitor how decision threshold affects fairness per group
...
- [ ] A. Denormalization: flatten feature schemas to reduce joins; trade storage for query speed
- [ ] B. Partitioning: shard features by entity_id; distribute load across servers
- [ ] C. Caching tiers: L1 (memory), L2 (Redis), L3 (database); serve from nearest tier
- [ ] D. Async writes: batch writes to offline store; don't block online serving on write latency
...
- [ ] A. Search space: define candidate architectures; large search spaces require more compute
- [ ] B. Early stopping: terminate unpromising architectures; saves compute
- [ ] C. Benchmark: compare AutoML-discovered models against hand-crafted baselines
- [ ] D. Transfer learning: initialize search with pre-trained models; speeds up search
...
- [ ] A. Experiment tracking: log all hyperparameters, seed, dataset version, code commit hash
- [ ] B. Versioning: version datasets, models, code; tag releases
- [ ] C. Environment isolation: Docker containers, Conda environments; ensure same dependencies
- [ ] D. MLflow / Weights & Biases: tools for tracking experiments; enable easy reproduction
...
- [ ] A. Metrics dashboards: AUC, latency, error rate in real-time; visualize trends
- [ ] B. Structured logging: log predictions with input/output/latency; enable debugging
- [ ] C. Distributed tracing: trace request through system components; identify bottlenecks
- [ ] D. Alerts: trigger on metric anomalies; page on-call engineers
...
- [ ] A. ML platforms/tools: standardized stacks (Vertex AI, Feature Store, etc.); enable self-service model development
- [ ] B. Policies and standards: documented best practices for data collection, model training, deployment
- [ ] C. Cross-functional collaboration: data eng, ML eng, product, ethics teams align on requirements
- [ ] D. Continuous learning: share learnings, incident postmortems, and best practices across teams
...
**TO SUBMIT AND SCORE YOUR TEST:**

1. After answering all 250 questions, replace all your selected answers' `[ ]` with `[x]`
2. Save this file as `MOCK_Q_A_COMPLETED.md`
3. Open `MOCK_Q_A_ANSWERS.md` in another window to reference correct answers
4. Use this Python script to compare and score (or manually compare):

```python
import re

# Read your answers
with open('MOCK_Q_A_COMPLETED.md', 'r') as f:
    your_answers = f.read()

# Read answer key
with open('MOCK_Q_A_ANSWERS.md', 'r') as f:
    answer_key = f.read()

# Extract checked answers (pattern: - [x] X. {text})
your_checked = re.findall(r'- [x] ([A-D]).', your_answers)
correct_checked = re.findall(r'- [x] ([A-D]).', answer_key)

# Score
score = sum(1 for y, c in zip(your_checked, correct_checked) if y == c)
total = len(correct_checked)
percentage = (score / total * 100) if total > 0 else 0

print(f"Your Score: {score}/{total} ({percentage:.1f}%)")
print(f"Passing Score (70%): {int(total * 0.7)}/{total}")

if percentage >= 70:
    print("✓ PASS - You're ready for the exam!")
else:
    print("✗ NEEDS REVIEW - Study weak domains and retake")
```

**OR compare manually:**
- Open this file and MOCK_Q_A_ANSWERS.md side-by-side
- For each question, check if your [x] answer matches the answer key [x]
- Count correct answers
- Score = (correct / 250) * 100
